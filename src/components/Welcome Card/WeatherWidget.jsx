...
const WeatherWidget = () => {
    ...
    const fetchWeather = async () =>{
        try{
            ...
            const res = await axios.get(`https://api.openweathermap.org/data/2.5/weather/?lat=${coords.latitude}&lon=${coords.longitude}&appid=${apiKey}&units=metric`);
    ...
    function getWeatherIcon(description){
        ...
        if(hour < 18 && hour > 5){
            ...
            if(description ==="Rain")
                return nightRainSVG
            if(description === "Thunderstorm")
                return nightThunderSVG
            if(description === "Clouds")
                return nightCloudSVG
            if(description === "Clear")
                return nightClearSVG
            if(description === "Atmosphere")
                return atmosphereSVG
            if(description === "Snow")
                return nightSnowSVG
  ...
  return (
    <div className="weatherWidget">
            <div className="tempLocation">
                <img src={getWeatherIcon(weatherDescription)} alt="Weather Icon" id="weatherIcon"/>
            ...
            <div className="weatherInfo-grid">
                ...
                <div className="grid-item">{Math.round(windspeed)}<span className="units">m/s</span></div>
                <div className="grid-item">{humidity}<span className="units">%</span></div>
                <div className="grid-item">{Math.round(feelsLikeTemp)}<span className="units">°C</span></div>
            </div>
            ...
            :
                ...
                <div className="updatedTime">{hour}:{min}</div>
            </div>
