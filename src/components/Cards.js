...
function Cards() {
  ...
  const [ todayTasks, setTodayTasks] = useLocalStorage('dashboard.todayTasks', []);
  ...
  const toggleTask = (id) =>{
    setTodayTasks(prevState => prevState.map(t => (t.id === id ? { ...t, checked: !t.checked} : t
      )))
  }
  const updateTask = (task) =>{
    ...
    setTodayTasks(prevState => prevState.map(t => (t.id === task.id ? { ...t, name: task.name} : t
      )))
      ///close the edit mode
      closeEditMode();
  }
  ...
  const [importantTasks, setImportantTasks] = useLocalStorage('tasks.starredTasks', []);
  const [mainTodoTasks, setMainTodoTasks] = useLocalStorage('tasks.mainTasks', [])
  const [completedTodoTasks, setCompletedTodoTasks] = useLocalStorage('tasks.completedTasks', [])
  ...
  const handleCheckedTask =(important)=>{
    ...
    setCompletedTodoTasks(prevState => prevState.map(t => (t.id === important.id ? { ...t, checked: !t.checked} : t
      )))
    //remove from starredTask array
    const timeout = setTimeout(function(){
      setImportantTasks(prevState => prevState.filter(t=> t.id !== important.id))}, 150);
    
    //remove from mainTask array 
    setMainTodoTasks(prevState => prevState.filter(t=> t.id !== important.id))
  }
  ...
  const updateGoal = (goal) =>{
    ...
    setGoals(prevState => prevState.map(g => (g.id === goal.id ? { ...g, name: goal.name} : g
      )))
      ///close the edit mode
      closeGoalEditMode();
  }
  ...
  const toggleGoal = (id) =>{
    setGoals(prevState => prevState.map(t => (t.id === id ? { ...t, checked: !t.checked} : t
      )))
  }
      ...
      <div className="cards-laptop-view">
        ...
        <h3>Important</h3>
          <div className="todo-important">
            ...
            {importantTasks && <ImportantList importantTasks={importantTasks} removeImportant={removeImportant} handleCheckedTask={handleCheckedTask} listContainerStyle={listContainerStyle}/>}
          </div>
        </div>
        ...
        <div className="card-item two">
          ...
          <div className="todo-today">
            ...
            {todayTasks && (
              <TodayTaskList todayTasks={todayTasks} 
                deleteTask={deleteTask}
                toggleTask={toggleTask}
                enterEditMode = {enterEditMode}
                isEditing={isEditing} editedTask={editedTask} updateTask={updateTask} closeEditMode={closeEditMode}
        ...
        <div className="card-item three">
          ...
          <div className="todo-goals">
            ...
            {goals && <GoalsList goals={goals} deleteGoal={deleteGoal} enterGoalEditMode={enterGoalEditMode} 
            isEditingGoal={isEditingGoal} editedGoal={editedGoal} updateGoal={updateGoal} closeGoalEditMode={closeGoalEditMode} toggleGoal={toggleGoal} listContainerStyle={listContainerStyle}/>}
          </div>
        </div>
      </div>
      <div className='cards-touchscreen-view'>
        <Swiper
         pagination={true} modules={[Pagination]} spaceBetween={10} className="mySwiper">
        <SwiperSlide>
          <div className="card-item one">
            <h3>Important</h3>
            <div className="todo-important">
              <ImportantForm addImportant={ addImportant }/>
              {importantTasks && <ImportantList importantTasks={importantTasks} removeImportant={removeImportant} handleCheckedTask={handleCheckedTask} />}
            </div>
          </div>
        </SwiperSlide>
        <SwiperSlide>
        ...
        <SwiperSlide>
          <div className="card-item three">
            ...
            <div className="todo-goals">
              ...
              {goals && <GoalsList goals={goals} deleteGoal={deleteGoal} enterGoalEditMode={enterGoalEditMode} 
              isEditingGoal={isEditingGoal} editedGoal={editedGoal} updateGoal={updateGoal} closeGoalEditMode={closeGoalEditMode} toggleGoal={toggleGoal}/>}
            </div>
          </div>
        </SwiperSlide>
      </Swiper>
