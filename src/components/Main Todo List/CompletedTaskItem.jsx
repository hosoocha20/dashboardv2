...
const CompletedTaskItem = ({completedTask, deleteCompletedTask, toggleTask, toggleCompletedTaskStarred, addStarredTask, removeStarredTask, toggleCompletedStarredTask}) => {
  ...
  return (
    <li className="completedTaskItem">
        <div className="completedItem-group">
            <input type="checkbox" className='mainCheckbox' checked={isChecked} name={completedTask.name} id={completedTask.id} 
        ...
        <div className="mainIcon-btns">
            <div className='btn mainDelete' aria-label={`Delete ${completedTask.name} Task`} >
                <RxTrash className='mainDelete-icon' size="17px" onClick={() => deleteCompletedTask(completedTask.id)}
                    onMouseOver={({target})=>target.style.color="dimGray"}  onMouseOut={({target})=>target.style.color="darkGray"} />
            </div>
            <div className="btn mainStar">
                    {isActive? <RxStarFilled  color="gold" size="21px" onClick={()=>{
                        handleStarredChange()}} />:
