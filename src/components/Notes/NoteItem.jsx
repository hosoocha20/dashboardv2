...
const NoteItem = ({ note, togglePinned, deleteNote, openNote, noteToOpen }) => {
  ...
  return (
    <div
      className={note.id === noteToOpen.id ? "noteItem opened" : "noteItem"}
    ...
    >
          ...
          {isHovering &&
            (isHoveringIcon ? (
              ...
              <RxTrash
                ...
                onMouseOut={handleMouseOutIcon}
                className="noteItem-trash-icon"
              />
            ))}
          {note.pinned ? (
            <BsFillPinAngleFill onClick={handlePinEvent} color="dimGray" />
          ) : (
            isHovering && (
              <BsFillPinAngleFill
      ...
      <div className="noteItem-sidebar">
        ...
        <div className="noteItem-sneakpeek">
          {note.body && note.body.substring(0, 35)}
        </div>
      </div>
    </div>
  );
