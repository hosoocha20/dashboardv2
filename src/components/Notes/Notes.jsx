...
const Notes = () => {
  ...
  const togglePinned = (note) => {
    ...
    setNotes((prevState) =>
      prevState.map((n) => (n.id === note.id ? { ...n, pinned: !n.pinned } : n))
    );
    if (isPinned) openNote(note);
  };
  const deleteNote = (id) => {
    goBackToList();
  ...
  const updateTitle = (id, newTitle) => {
    ...
    setNotes((prevState) =>
      prevState.map((n) => (n.id === id ? { ...n, title: newTitle } : n))
    );
  };
  const updateBody = (id, newBody) => {
    setIsEditingNote(true);
    setNotes((prevState) =>
      prevState.map((n) => (n.id === id ? { ...n, body: newBody } : n))
    );
  };

  const searchInputCount = (e) => {
    let c = notes.filter((i) =>
  ...
  return (
    <div className="notes-grid-container">
      <div
        className={
          isNoteOpened
            ? "notes-sidebar noteList-slideOut"
            : isNotePageClosed
            ? "notes-sidebar noteList-slideIn"
            : "notes-sidebar"
        }
      >
      ...
      <div
        className={
          isNoteOpened
            ? "note-page-container notePage-slideIn"
            : isNotePageClosed
            ? "note-page-container notePage-slideOut"
            : "note-page-container"
        }
      >
        {notes.map((n) =>
